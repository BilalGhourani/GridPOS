...
data class InvoiceHeader(
    ...
    @ServerTimestamp
    var invoiceHeadTimeStamp: Date? = Date(),
    ...
    override fun getName(): String {
        val transactionCode = invoiceHeadTtCodeName ?: invoiceHeadTtCode
        val table = if (invoiceHeadTaName.isNullOrEmpty()) "" else invoiceHeadTaName
        val transNo =
            if (invoiceHeadTransNo.isNullOrEmpty()) "$invoiceHeadOrderNo" else if (transactionCode.isNullOrEmpty()) "$invoiceHeadTransNo" else "$transactionCode$invoiceHeadTransNo"
    ...
    override fun getMap(): Map<String, Any?> {
        return mapOf(
            ...
            "hi_timestamp" to Timestamp.valueOf(
                DateHelper.getDateInFormat(
                    invoiceHeadTimeStamp?:Date(),
                    "yyyy-MM-dd HH:mm:ss"
                )
            ),
    ...
    fun didChanged(invoiceHeader: InvoiceHeader): Boolean {
        return !invoiceHeader.invoiceHeadNote.equals(invoiceHeadNote) || !invoiceHeader.invoiceHeadCashName.equals(
            invoiceHeadCashName
        ) || invoiceHeader.invoiceHeadDiscount != invoiceHeadDiscount || invoiceHeader.invoiceHeadDiscountAmount != invoiceHeadDiscountAmount
    }
