...
interface InvoiceHeaderDao {
    ...
    // Get all Invoice Headers as stream.
    @Query("SELECT * FROM `in_hinvoice` WHERE hi_tt_code = :type AND hi_cmp_id=:companyId AND (hi_orderno IS NOT NULL OR hi_orderno != '') ORDER BY hi_datetime DESC LIMIT 1")
    ...
    @Query("SELECT * FROM `in_hinvoice` WHERE hi_tt_code = :type AND hi_cmp_id=:companyId AND (hi_transno IS NOT NULL OR hi_transno != '') ORDER BY hi_datetime DESC LIMIT 1")
