    ...
    fun fetchInvoiceByTable(tableNo: String) {
        if (tableNo.isEmpty()) {
            tablesState.value = tablesState.value.copy(
                warning = "Please enter table number!", isLoading = false
            )
            return
        }
        tablesState.value = tablesState.value.copy(
        ...
        viewModelScope.launch(Dispatchers.IO) {
            invoiceHeaderRepository.getInvoiceByTable(tableNo, object : OnResult {
                ...
                override fun onFailure(
                    ...
                    viewModelScope.launch(Dispatchers.Main) {
                        tablesState.value = tablesState.value.copy(
                            warning = "Failed to retrieve related invoice!"
                        )
                    }
                }
            })
